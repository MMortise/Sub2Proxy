...
例如：订阅里有「美国 1」「英国 1」，你可以让 `http://127.0.0.1:27100` 固定走美国、`http://127.0.0.1:27101` 固定走英国，内网其他程序按需指定出口。
...
登录后：添加订阅 → 节点列表测速 → 建端口映射 → 内网程序用 `http://<主机>:27100` 等出口。
...
二进制直接在宿主机监听 `27000` + `27100-27199`：无需 Docker、无端口发布开销、无挂载权限问题。默认装到**当前仓库所在目录**（在哪 clone 就装哪，二进制与 `data/` 都落在仓库根、已被 gitignore 忽略）。要换目录用 `PREFIX` 覆盖：
...
| `27100–27199` | 代理映射段（100 个端口），每个映射占一个 |

Docker 端口发布是容器创建时静态确定的，所以 `27000` + `27100-27199` 一次性发布（共 101 个）。**若修改 `config.yaml` 的 `port_range`，必须同步修改 `docker-compose.yml` 的端口发布段**，否则新端口无法从宿主机访问。（映射段刻意控制在 100 个端口，避免 bridge 模式为上千端口创建海量 docker-proxy 进程。）

默认 compose 把端口绑到所有网卡，局域网其他设备可直接访问。要限制为**仅本机**，给 `docker-compose.yml` 两条 `ports` 都加 `127.0.0.1:` 前缀。⚠️ 代理端口（27100–27199）只有在映射里设置了用户名/密码时才有认证，否则局域网内任何人都能使用。
...
port_range: [27100, 27199]       # 映射端口自动分配范围（100 端口；与 compose 发布段保持一致）
...
- **不要把它裸奔在公网。** 若必须公网访问，请在前面放置反向代理并启用 TLS，且强烈建议不要暴露 27100–27199 代理段。
