...
二进制直接在宿主机监听 `27000-27999`：无需 Docker、无端口发布开销、无挂载权限问题。默认装到 `/opt/sub2proxy`（`config.yaml` 与节点缓存在 `/opt/sub2proxy/data`）。要换目录，用 `PREFIX` 覆盖：

```bash
PREFIX=/home/sub2proxy sudo -E deploy/install.sh   # 装到 /home/sub2proxy
```

`install.sh` 会把 `data_dir` 写成安装目录下的 `data`，systemd 单元按实际路径生成——无需手动对齐。
